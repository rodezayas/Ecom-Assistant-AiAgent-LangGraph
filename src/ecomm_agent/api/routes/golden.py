...
def get_by_slug(slug: str) -> GoldenQuery:
    """Get golden query by slug."""
    if not is_supabase_configured():
        raise HTTPException(status_code=503, detail="Supabase not configured")
    item = get_golden_query_by_slug(slug)
    if item is None:
        raise HTTPException(status_code=404, detail=f"golden query '{slug}' not found")
    return item


@router.get("/{query_id}", response_model=GoldenQuery)
def get_by_id(query_id: str) -> GoldenQuery:
...
def create_golden(payload: GoldenQueryCreate) -> GoldenQuery:
    """Create a golden query (requires service_role)."""
    if not is_supabase_configured():
        raise HTTPException(status_code=503, detail="Supabase not configured")
...
def list_evaluation_runs(limit: int = Query(default=20, ge=1, le=100)) -> list[dict]:
    ...
    try:
        ...
        raise HTTPException(status_code=500, detail=f"evaluation_runs table missing? run docs/evaluation-sql.md. error: {exc}")


@router.get("/evaluation/results/{run_id}")
def list_evaluation_results(run_id: str) -> list[dict]:
    ...
    try:
        ...
        raise HTTPException(status_code=500, detail=f"evaluation_results table missing? error: {exc}")
